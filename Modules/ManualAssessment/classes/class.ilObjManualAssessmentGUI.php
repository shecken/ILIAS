...
class ilObjManualAssessmentGUI extends ilObjectGUI
{
	...
	public function getTabs()
	{
		...
		if ($access_handler->checkAccessToObj($this->object, 'read')) {
			$this->tabs_gui->addTab(self::TAB_INFO, $this->lng->txt('info_short'), $this->getLinkTarget('info'));
		}
		if ($access_handler->checkAccessToObj($this->object, 'write')) {
			$this->tabs_gui->addTab(self::TAB_SETTINGS, $this->lng->txt('settings'), $this->getLinkTarget('settings'));
		}
		...
		if (($access_handler->checkAccessToObj($this->object, 'read_learning_progress')
			...
			$this->tabs_gui->addTab(self::TAB_LP, $this->lng->txt('learning_progress'), $this->ctrl->getLinkTargetByClass('illearningprogressgui'));
		}
		...
		if ($access_handler->checkAccessToObj($this->object, 'edit_permission')) {
			$this->tabs_gui->addTarget(self::TAB_PERMISSION, $this->ctrl->getLinkTargetByClass('ilpermissiongui', 'perm'), array(), 'ilpermissiongui');
		}
		parent::getTabs();
	}
