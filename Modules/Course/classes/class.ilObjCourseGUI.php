...
class ilObjCourseGUI extends ilContainerGUI
{
	...
	* @access public
	*/
	public function ilObjCourseGUI()
	{
	...
	*/
	public function infoScreen()
	{
		...
		if ($this->object->getContactEmail()) {
			...
			foreach ($emails as $email) {
				...
				$etpl->setVariable("EMAIL_LINK", ilMailFormCall::getLinkTarget(
					$info,
					'showSummary',
					array(),
	...
	public function askDeleteStarterObject()
	{
		global $tpl;
	...
	public function deleteStarterObject()
	{
		$this->checkPermission('write');
	...
	public function selectStarterObject()
	{
		global $tpl;
	...
	public function addStarterObject()
	{
		global $rbacsystem;
...
//				$lp_status_options->setRequired(true);
	...
	public function updateCourseIconsObject()
	{
		...
		if ($form->checkInput()) {
			...
			if ($this->ilias->getSetting("custom_icons")) {
				...
				$this->object->saveIcons(
					$_FILES["cont_big_icon"]['tmp_name'],
		...
		switch ($a_tab) {
			case "properties":
				...
				if (gevCourseUtils::getInstanceByObj($this->object)->isFlexibleDecentrallTraining()) {
					...
					$this->tabs_gui->addSubTabTarget(
						'gev_dec_crs_building_block_sub_menu_title',
	...
	*/
	...
	public function downloadArchivesObject()
	{
		global $rbacsystem;
	...
	protected function membersObject()
	{
		...
		*/
		...
		$ilToolbar->addSeparator();
		$ilToolbar->addButton($this->lng->txt("associated_users"), "ilias.php?ref_id=".$_GET["ref_id"]."&cmd=trainer&baseClass=gevMemberListDeliveryGUI");
		$ilToolbar->addButton($this->lng->txt("gev_uvg"), "ilias.php?ref_id=".$_GET["ref_id"]."&cmd=uvg&baseClass=gevMemberListDeliveryGUI");
		if (!in_array($utils->getType(), array("Webinar", "Spezialistenschulung Webinar"))) {
			$ilToolbar->addButton($this->lng->txt("gev_signature_list"), "ilias.php?ref_id=".$_GET["ref_id"]."&cmd=download_signature_list&baseClass=gevMemberListDeliveryGUI");
		}
		if (in_array($utils->getType(), array("Webinar", "Spezialistenschulung Webinar"))) {
			$ilToolbar->addButton($this->lng->txt("gev_csn"), "ilias.php?ref_id=".$_GET["ref_id"]."&cmd=csn&baseClass=gevMemberListDeliveryGUI");
		}
		if ($utils->canBuildDeskDisplays()) {
			$ilToolbar->addButton($this->lng->txt("gev_desk_displays"), "ilias.php?ref_id=".$_GET["ref_id"]."&baseClass=gevDeskDisplaysDeliveryGUI");
		}

		// i know this has timezone issues...
		if ($utils->userCanCancelCourse($ilUser->getId())) {
			$ilToolbar->addButton($this->lng->txt("gev_cancellation"), $this->ctrl->getLinkTarget($this, "confirmTrainingCancellation"));
		}
		// gev-patch end
	...
	public function updateParticipantsStatus($type, $visible_members, $passed, $notification, $blocked)
	{
	...
	public function __showWaitingList()
	{
		...
		if ($this->object->waiting_list_obj->getCountUsers()) {
			...
			foreach ($this->object->waiting_list_obj->getAllUsers() as $waiting_data) {
				...
				if ($tmp_obj = ilObjectFactory::getInstanceByObjId($waiting_data['usr_id'], false)) {
					...
					foreach (ilObjCourseGrouping::_getGroupingCourseIds(
						$this->object->getRefId(),
						$this->object->getId()
					) as $course_data) {
	...
	public function updateMemberObject()
	{
	...
	public function performRemoveFromWaitingListObject()
	{
		global $rbacsystem;
	...
	public function deleteMembersObject()
	{
		...
		if (!$ilAccess->checkAccess('edit_permission', '', $this->object->getRefId()) and
			!ilCourseParticipants::_getInstanceByObjId($this->object->getId())->isAdmin($ilUser->getId())
		) {
	...
	*/
	public function getTabs(&$tabs_gui)
	{
		...
		if ($ilAccess->checkAccess("write", "", $this->ref_id)) {
			...
			$tabs_gui->addTab("mailing", $this->lng->txt("mailing"), $this->ctrl->getLinkTargetByClass(array("ilobjcoursegui", "gevcrsmailinggui"), ""));
		}
		// gev-patch end
		...
		} elseif ($this->object->getShowMembers() == $this->object->SHOW_MEMBERS_ENABLED and
			$is_participant
		) {
			$tabs_gui->addTarget(
				"members",
		...
		} elseif ($this->object->getMailToMembersType() == ilCourseConstants::MAIL_ALLOWED_ALL and
	...
	public function fetchPrintSubscriberData($a_members)
	{
	...
	public function fetchPrintMemberData($a_members)
	{
		global $ilAccess,$lng;
	...
	public function printMembersObject()
	{
		global $ilTabs;
	...
	protected function initAttendanceList()
	{
		...
		$list->setTitle(
			$this->lng->txt('crs_members_print_title'),
	...
	public function printMembersOutputObject()
	{
		$list = $this->initAttendanceList();
		$list->initFromForm();
	...
	public function __showRemoveFromWaitingListTable($a_result_set)
	{
	...
	public function __showDeleteSubscriberTable($a_result_set)
	{
		$tbl =& $this->__initTableGUI();
		$tpl =& $tbl->getTemplateObject();
	...
	public function __showWaitingListTable($a_result_set, $a_waiting_list_ids = null)
	{
	...
	public function mailMembersObject()
	{
		...
		foreach ((array) $sorted_role_ids as $role_id) {
			...
			$this->tpl->setVariable("CHECK_MAILBOX", ilUtil::formCheckbox(
				1,
				'roles[]',
...
//						$this->checkPermission("read");
	...
	public function cciObjectivesObject()
	{
		$this->initCourseContentInterface();
		$this->cci_obj->cci_setContainer($this);
	...
	public function cciObjectivesEditObject()
	{
	...
	public function cciObjectivesAskResetObject()
	{
		$this->initCourseContentInterface();
		$this->cci_obj->cci_setContainer($this);
	...
	public function cciResetObject()
	{
		global $ilUser;
	...
	* goto target course
	*/
	public function _goto($a_target, $a_add = "")
	{
	...
	public function toUnix($date, $time = array())
	{
...
*/
	...
	*/
	...
	protected function confirmTrainingCancellationObject()
	{
		...
		$conf->addItem("crs", $this->object->getId(), $util->getTitle()." (".$util->getFormattedAppointment().")");
