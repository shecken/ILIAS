...
class ilObjStudyProgrammeSettingsGUI
{
        ...
        return $this->input_factory->container()->form()->standard(
            ...
            $this->refinery_factory->custom()->transformation(
                function ($values) use ($prg) {
                    ...
                    $prg_not_restarted_by_user_days = (int)$values[5][self::PROP_CRON_JOB_PRG_NOT_RESTARTED];
                    $prg->setReminderNotRestartedByUserDays($prg_not_restarted_by_user_days);
                    $prg_processing_ends_not_successful_days = (int)$values[5][self::PROP_CRON_JOB_PROCESSING_ENDS_NOT_SUCCESSFUL];
    ...
    protected function getCronJobConfiguration(ilObjStudyProgramme $prg)
    {
        ...
        $prg_not_restarted_input =
            ...
            ->withValue($prg->getReminderNotRestartedByUserDays())
        ;
        ...
        $prg_processing_ends_no_success_input =
            $prg_not_restarted_input
                ...
                ->withValue($prg->getProcessingEndsNotSuccessfulDays())
        ;

        return [
            self::PROP_CRON_JOB_PRG_NOT_RESTARTED => $prg_not_restarted_input,
            self::PROP_CRON_JOB_PROCESSING_ENDS_NOT_SUCCESSFUL => $prg_processing_ends_no_success_input
        ];
    }
