...
class gevDecentralTrainingGUI
{
	...
	protected function finalizeTrainingCreation()
	{
		...
		$trainer_ids = array_map(function ($inp) {
			return (int)$inp;
	...
	protected function updateBuildingBlock()
	{
		...
		if (isset($_POST["venue"])) {
			...
			$new_vals["venue_room_nr"] = $_POST["venue_room_nr"] ? $_POST["venue_room_nr"] : null;
	...
	protected function showSettings($a_form = null)
	{
		...
		$title = new catTitleGUI("gev_dec_training_settings_header", "gev_dec_training_settings_header_note", "GEV_img/ico-head-create-decentral-training.png");
	...
	protected function getSettingsFromForm(gevCourseUtils $a_target, $a_form, $a_template_id)
	{
		...
		if ($is_flexible) {
			...
			$start_time_aux = explode(':', $time['start']['time']);
			$end_time_aux = explode(':', $time['end']['time']);

			$start_time = $start_time_aux[0].':'.$start_time_aux[1];
			$end_time = $end_time_aux[0].':'.$end_time_aux[1];
			...
			foreach ($a_target->getSchedule() as $day => $tpl_duration) {
				...
				$start = new DateTime();
				$start->setTimestamp($unix_start);
				$end = new DateTime();
				$end->setTimestamp($unix_start + $duration_m * 60);
				$periods[] = $start->format('H:i').'-'.$end->format('H:i');
			}
			$first_day_begin = explode('-', current($periods))[0].':00';
			$last_day_end = explode('-', end($periods))[1].':00';
			$start_datetime = new ilDateTime($start_date_date.' '.$first_day_begin, IL_CAL_DATETIME);
			$aux = new DateTime($start_date_date);
			$aux->add(new DateInterval('P'.(count($periods)-1).'D'));
			$end_datetime = new ilDateTime($aux->format('Y-m-d').' '.$last_day_end, IL_CAL_DATETIME);
		}
	...
	protected function buildTrainingOptionsFormStable($a_fill, array $a_form_values = null)
	{
		...
		*************************/
		...
		foreach ($schedule as $day => $duration) {
			$hi = new ilHiddenInputGUI('schedule_durations_'.$day);
			$hi->setValue($a_form_values['schedule_durations_'.$day]);
			$form->addItem($hi);
		}
	...
	protected function checkDecentralTrainingConstraints(&$a_form, $a_template_id)
	{
		...
		if ($a_template_id == $presence_flexible_tpl_id || $a_template_id == $webinar_flexible_tpl_id) {
			...
			foreach ($crs_utils->getSchedule() as $day => $time_period) {
				...
				$time_s = $a_form->getInput('schedule_starts_'.$day)['time'];
				$aux = explode(':', $time_s);
				$time_s_h = $aux[0];
				$time_s_m = $aux[1];
	...
	protected function confirmTrainingCancellation()
	{
		...
		$crs_utils = gevCourseUtils::getInstance($_POST["obj_id"]);
		$conf->addItem("obj_id", $_POST["obj_id"], $crs_utils->getTitle()." (".$crs_utils->getFormattedAppointment().")");
