...
class gevUserUtils
{
	...
	public static function getInstanceByObj(ilObjUser $a_user_obj)
	{
	...
	public static function getInstanceByObjOrId($a_user)
	{
	...
	public static function userIdExists($a_user_id)
	{
		global $ilDB;
	...
	public static function getEduBioLinkFor($a_target_user_id)
	{
	...
	public function getBookedAndWaitingCourseInformation()
	{
		...
		foreach ($booked_amd as $key => $value) {
			...
			$booked_amd[$key]["absolute_cancel_date"] = gevCourseUtils::mkDeadlineDate($value["start_date"], $value["absolute_cancel_date"]);
		...
		foreach ($waiting_amd as $key => $value) {
			...
			$waiting_amd[$key]["cancel_date"] = gevCourseUtils::mkDeadlineDate($value["start_date"], $value["cancel_date"]);
			$waiting_amd[$key]["absolute_cancel_date"] = gevCourseUtils::mkDeadlineDate($value["start_date"], $value["absolute_cancel_date"]);
	...
	public function getMyAppointmentsCourseInformation($a_order_field = null, $a_order_direction = null)
	{
		...
		if ($a_order_field) {
			...
			$crss_amd = gevAMDUtils::getInstance()->getTable(
				$crss_ids,
				$crs_amd,
				array("pstatus.state pstate"),
	...
	public function getEmployeeIdsForBookingView()
	{
		...
		$e_ids = array_unique(array_merge(gevOrgUnitUtils::getEmployeesIn($e_ous), gevOrgUnitUtils::getAllPeopleIn($a_ous)));
	...
	public static function getFullNames($a_user_ids)
	{
		global $ilDB;
	...
	*/
	...
	{
		return $this->hasRoleIn("Admin-TA");
	}
	...
	*/
	...
	public function canBookCourseDerivedFromTemplate($a_tmplt_ref_id)
	{
		...
		$sql =  "SELECT COUNT(*) cnt "
			   ...
			   ."   
			   		AND (    ".$this->db->in("cb.status", array(ilCourseBooking::STATUS_BOOKED, ilCourseBooking::STATUS_WAITING), false, "integer")
	...
	public static function removeInactiveUsers($a_usr_ids)
	{
		global $ilDB;
	...
	public function getOrgUnitsWhereUserCanViewEduBios()
	{
		...
		$rs = array_map(function ($v) {
			return $v["ref_id"];
		}, gevOrgUnitUtils::getAllChildren($r));
	...
	protected static $hist_position_keys = null;

	public static function getPositionKeysFromHisto()
	{
	...
	public static function setUserActiveState($user_id, $active)
	{
		require_once("Services/User/classes/class.ilObjUser.php");
...
/*					case "auf Warteliste":
	...
	public static function getBuildingBlockPoolsUserHasPermissionsTo($user_id, array $permissions)
	{
		global $ilDB;
	...
	public static function getBuildingBlockPoolsTitleUserHasPermissionsTo($user_id, array $permissions)
	{
		global $ilDB;
	...
	/**
	 * Prevent registered user from user data validation prompt
	 *
	 * @return null
	 */
	public function preventUserDataValidation()
	{
	...
	/**
	 * Get plugin object of user data validation prompt
	 *
	 * @return ilPlugin | null
	 */
	protected function getUserValidationPromptPlugin()
	{
		if ($this->user_data_val_prompt_plugin === null) {
			$user_data_val_prompt_plugin = ilPlugin::getPluginObject(IL_COMP_SERVICE, "UIComponent", "uihk", "UserDataValidationPrompt");
			if ($user_data_val_prompt_plugin && $user_data_val_prompt_plugin->active) {
				$this->user_data_val_prompt_plugin = $user_data_val_prompt_plugin;
			}
		}

		return $this->user_data_val_prompt_plugin;
	}
}
