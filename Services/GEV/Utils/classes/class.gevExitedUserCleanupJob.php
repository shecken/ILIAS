...
class gevExitedUserCleanupJob extends ilCronJob
{
	...
	public function run()
	{
		...
		$sap_pnr_field_id = $gev_settings->getUDFFieldId(gevSettings::USR_UDF_PERSONAL_ID);
		...
		$res = $ilDB->query("SELECT ud.usr_id "
						   ...
						   ."  LEFT JOIN udf_text sap_users"
						   ."    ON sap_users.usr_id = ud.usr_id"
						   ."   AND sap_users.field_id = ".$ilDB->quote($sap_pnr_field_id, "integer")
						   ." WHERE active = 1 "
						   ."	AND udf.value < CURDATE()"
						   ."	AND (sap_users.value IS NULL"	// non empty values for pnr
						   ."		OR sap_users.value REGEXP '^[[:space:]]*$')");
