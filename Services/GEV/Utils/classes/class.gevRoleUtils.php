...
class gevRoleUtils
{
	...
	public function getGlobalRolesWithDesc()
	{
		if ($this->global_roles === null) {
			$roles = $this->getRbacReview()->getGlobalRoles();

			$res = $this->db->query("SELECT obj_id, title, description FROM object_data "
								   ." WHERE ".$this->db->in("obj_id", $roles, false, "integer"));

			$this->global_roles = array();
			while ($rec = $this->db->fetchAssoc($res)) {
				$this->global_roles[(int)$rec["obj_id"]] = array("title" => $rec["title"], "description" => $rec["description"]);
			}
		}
