...
class gevWBDRequestKontoAenderung extends WBDRequestKontoAenderung {
	use gevWBDRequest;
	...
	protected function __construct($data) {
		...
		if(!empty($errors)) {
			throw new myLogicException(
				"gevWBDRequestKontoAenderung::__construct:checkData failed",
				0,
				null,
				$errors
			);
	...
	public static function getInstance(array $data) {
		...
		try {
			return new gevWBDRequestKontoAenderung($data);
	...
	*/
	public function createWBDSuccess($response) {
		$this->wbd_success = new gevWBDSuccessKontoAenderung(
			$response,
			$this->row_id
		);
	}
