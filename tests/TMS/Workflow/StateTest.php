...
class TMS_Workflow_StateTest extends PHPUnit_Framework_TestCase {
	public function test_getStepData_throws() {
		$state = new Workflow\State("id",0);
		$catched = false;
		try {
			$state->getStepData(0);
	...
	public function test_getStepData_by_constructor() {
		...
		$state = new Workflow\State("id",0, [0 => $encoded]);
	...
	public function test_getWorkflowId() {
		$state = new Workflow\State("id",0, []);

		$this->assertEquals("id", $state->getWorkflowId());
	}
}
