...
class ilResultPDF
{
	...
	protected function createPDF()
	{
		...
		$pdf->title($this->fillPlaceholder($this->encodeSpecialChars($this->settings->getReportTitle())));
	...
	protected function fillPlaceholder($text)
	{
		$text = str_replace("[VORNAME]", $this->settings->getFirstname(), $text);
		$text = str_replace("[NACHNAME]", $this->settings->getLastname(), $text);
		$text = str_replace("[KARRIEREZIEL]", $this->actions->getCareerGoalTitle($this->settings->getCareerGoalId()), $text);
