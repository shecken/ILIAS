...
class ilObjReportExamBio extends ilObjReportBase2
{
	...
	protected function createLocalReportSettings()
	{
		...
		if ($this->getRefId() && null === ilObject::_lookupDeletedDate($this->getRefId())) {
	...
	public function prepareTable(catSelectableReportTableGUI $table)
	{
		...
		if ($this->forTrainerView()) {
			$table->defineFieldColumn($this->plugin->txt('lastname'), 'lastname', array('lastname' => $this->space->table('usr')->field('lastname')))
				->defineFieldColumn($this->plugin->txt('firstname'), 'firstname', array('firstname' => $this->space->table('usr')->field('firstname')))
				->defineFieldColumn($this->plugin->txt('orgunit'), 'orgunit', array('orgunit' => $this->space->table('orgu_all')->field('orgus')), true);
		}
		...
		$table->defineFieldColumn(
			$this->plugin->txt('test_title'),
			'test_title',
			array('test_title' => $test_title)
		)
			->defineFieldColumn(
				$this->plugin->txt('test_date'),
				'test_date',
				array('test_date' => $testrun_finished_ts),
				true
			)
			->defineFieldColumn(
				$this->plugin->txt('passed'),
				'passed',
				array('passed' => $passed)
			)
			...
			->defineFieldColumn(
				$this->plugin->txt('number_of_runs'),
				'runs',
				array('runs' => $this->tf->countAll('number_of_runs')),
				true
			);
	...
	{
		return array_map(function ($nummeric) {
			return (int)$nummeric;
		}, $this->target_user_ids);
	}
