...
class ilObjReportASTD extends ilObjReportBase
{
	...
	protected function prepareQueryComponents($a_query)
	{
		$this->categories = array(	'astd_hours_not_self_learn' 	=>	" SUM( IF(".$this->gIldb->in('type', array('Virtuelles Training','Präsenztraining','Webinar'), false, 'text').", "
									...
									,'astd_hours_self_learn' 		=>	" SUM( IF((type = 'Selbstlernkurs' OR type = 'Coaching') AND credit_points IS NOT NULL, 4*GREATEST(credit_points,0)/3, 0)) "
	...
	{
		$filter	->dateperiod("period", $this->plugin->txt("period"), $this->plugin->txt("until"), "c.end_date", "c.end_date", date("Y")."-01-01", date("Y")."-12-31")
				->checkbox('no_wbd_imported', $this->plugin->txt("filter_no_wbd_imported"), " ucs.crs_id > 0", " TRUE ")
	...
	public function buildQueryStatement()
	{
		...
		$in_role_ids = array_map(function ($rol_title) use ($role_utils) {
			return $role_utils->getRoleIdByName($rol_title);
		}, $in_role);

		$not_in_role = $this->hierarchy["astd_middle_managers"];
		$not_in_role_ids = array_map(function ($rol_title) use ($role_utils) {
			return $role_utils->getRoleIdByName($rol_title);
		}, $not_in_role);
